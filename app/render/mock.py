    ...
    config = json.dumps(
        {
            ...
            "uiEndpoint": f"{settings.api_prefix}/chat",
            "logoUrl": settings.resolved_server_logo_url,
            # Root Cause vs Logic: the browser shell is served at `/chat`; we keep
            # `mockUiEndpoint` for older clients that still read that key.
            "mockUiEndpoint": f"{settings.api_prefix}/chat",
            "assetBaseUrl": f"{settings.api_prefix}/chat/assets",
            "publicBaseUrl": f"{settings.api_prefix}/chat/public",
        }
    )
    replacements = {
        "__HTH_MOCK_UI_LOGO_HREF__": settings.resolved_server_logo_url,
        "__HTH_MOCK_UI_STYLE_HREF__": f"{settings.api_prefix}/chat/assets/styles.css",
        "__HTH_MOCK_UI_SCRIPT_SRC__": f"{settings.api_prefix}/chat/assets/app.js",
