...
- For public finance/transparency questions, use accounting intent and plan `accounting_account_search`, `accounting_financial_snapshot`, or `accounting_expense_workflow`.
- When a lookup is missing or ambiguous, prefer a close existing match only when confidence is high; otherwise tell the user the item does not exist yet and ask whether they want to create a new account/slug/expense draft before taking a create action.
